    ...
    public class SmtpSocket
    {
        ...
        public async Task<SmtpResponse> EstablishConnection()
        {
            try
            {
                ...
                if (this.isSsl)
					//await socket.ConnectAsync(this.hostName, this.port.ToString(), SocketProtectionLevel.Ssl); Deprecated on Win 8.1
					await socket.ConnectAsync(this.hostName, this.port.ToString(), SocketProtectionLevel.Tls12);
				else
